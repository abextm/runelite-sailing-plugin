...
class Trial
{
	private static final int PREV_SEGMENTS = 2;
	private static final int MIN_SEGMENTS = 3;
	private static final int MAX_DISTANCE = 180 * Perspective.LOCAL_TILE_SIZE;
	...
	public Trial(int dbrow, int tier, Builder builder)
	{
		...
		for (int i = 0; i < builder.checkpoints.size(); i++)
		{
			...
			checkpoint.start = builder.checkpointPoints.get(Math.max(i - PREV_SEGMENTS, 0)).start;
			checkpoint.end = builder.checkpointPoints.get(Math.min(i + MIN_SEGMENTS, this.checkpoints.size() - 1)).end;
			...
			if (seg != null)
			{
				...
				double dist = 0;
				for (int j = Math.max(1, start); j < seg.size(); j++)
				{
					var b = seg.get(j);
					if (dist > MAX_DISTANCE)
					{
						checkpoint.end = Math.max(checkpoint.end, b.start);
						break;
					}
